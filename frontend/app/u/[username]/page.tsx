...
async function load(username: string) { try { return await serverApi<Profile>(`/profiles/${encodeURIComponent(username)}`); } catch (error) { if (error instanceof ApiRequestError && error.status === 404) notFound(); throw error; } }
