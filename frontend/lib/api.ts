...
export async function authorizedApi<T>(
  path: string,
  init?: RequestInit,
): Promise<T> {
  ...
  const send = async (forceRefresh: boolean) => {
    ...
    headers.set(
      "authorization",
      `Bearer ${await user.getIdToken(forceRefresh)}`,
    );
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  return parse<T>(
    await requestResponse(path, { ...init, headers: jsonHeaders(init) }),
  );
}
...
async function requestResponse(
  ...
  try {
    ...
    if (error instanceof DOMException && error.name === "TimeoutError")
      throw new ApiRequestError(
        "Sunucu zamanında yanıt vermedi. Biraz bekleyip tekrar dene.",
        0,
        "REQUEST_TIMEOUT",
      );
    throw new ApiRequestError(
...
async function parse<T>(response: Response): Promise<T> {
  ...
  try {
    payload = text ? (JSON.parse(text) as ApiEnvelope<T> | ApiError) : null;
  } catch {
    /* Upstream may return an HTML error page. */
  }
  if (!response.ok) {
    ...
    throw new ApiRequestError(
      apiError?.message ??
        (response.status >= 500
          ? "Sunucu geçici olarak yanıt veremiyor. Lütfen tekrar dene."
          : "İstek tamamlanamadı."),
      response.status,
      apiError?.code,
      apiError?.requestId,
